...
# Ensure NLTK stopwords are downloaded (first use)
try:
...
def extract_requirements(jd_text):
    ...
    # Gets phrases ordered from best to worst
    ranked_phrases = r.get_ranked_phrases()
    # Optionally limit to top N or filter by length
    requirements = [phrase.strip() for phrase in ranked_phrases if len(phrase.split()) > 1]
    return requirements
